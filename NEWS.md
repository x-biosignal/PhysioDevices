...
* Validated `readGoogleFit()`'s granular "All Data" JSON path against a **real**
  ...
  real-data harness in `planning/realdata-validation/` now also covers the
...
* Found by validating `readOura()` (and `readGoogleFit()`) against real public
  ...
  regression tests added; the reproducible real-data harness in
  `planning/realdata-validation/` now covers Apple, Fitbit, Oura and Google Fit.
...
* Found by validating `readAppleHealth()` and `readFitbit()` against real public
  ...
  (`test-io-realdata-variants.R`); the reproducible real-data script lives in
  `planning/realdata-validation/`.
